...
class CompostTokenMock {
  ...
  mint(caller: string, amount: number, recipient: string, metadata: string): ClarityResponse<boolean> {
    ...
    if (recipient === "deployer") { // Assuming deployer is restricted as in contract
...
describe("CompostToken Contract", () => {
  let contract: CompostTokenMock;

  beforeEach(() => {
    contract = new CompostTokenMock();
    vi.resetAllMocks();
  });

  it("should initialize with correct token metadata", () => {
    expect(contract.getName()).toEqual({ ok: true, value: "CompostToken" });
    expect(contract.getSymbol()).toEqual({ ok: true, value: "COMPOST" });
  ...
  it("should allow minter to mint tokens with metadata", () => {
    ...
    expect(mintRecord).toEqual({
      ...
      value: expect.objectContaining({
        ...
        metadata: "Composted 100kg organic waste",
      }),
    });
  });
